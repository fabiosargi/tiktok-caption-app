 ...
 * thumbnail_timestamp_ms, se houver) + ids das contas (+ scheduled_at, se for
 * uma publicação agendada) -> publica na hora, ou agenda dentro das próprias
 * plataformas se scheduled_at for enviado
 *
 ...
 * longo) aceitam uma imagem customizada (thumbnail_url) — que aqui é sempre um frame
 * real do próprio vídeo (escolhido pelo FrameSelector) com um texto curto por cima,
 * nunca uma imagem desenhada por IA generativa. Por isso mandamos os dois campos
 * juntos no mesmo item de mídia sempre que tiver algo pra mandar: cada plataforma usa
 * o que ela suporta e ignora o resto.
 *
 * Sobre agendamento: a própria API do Post for Me suporta agendar a publicação
 * (campo scheduled_at, um horário no futuro em UTC) — quando enviado, são as
 * plataformas (TikTok/Instagram/YouTube) que guardam e disparam o post sozinhas,
 * sem precisar que este aplicativo continue aberto até a hora marcada.
 *
 * Os extras (thumbnail e agendamento) nunca travam a publicação: se a geração ou
 * o upload da thumbnail falhar por qualquer motivo, a publicação do vídeo segue
 * normalmente, só que sem capa personalizada.
 */
                ...
                uploadMedia(apiKey, videoBytes, "video/mp4") { videoUploadResult ->
                    videoUploadResult.onSuccess { videoUrl ->
                        if (thumbnailBytes == null) {
                            createPost(
                                apiKey, caption, videoUrl, null, thumbnailTimestampMs, scheduledAt, accounts.map { it.id }
                        ) { postResult ->
                        ...
                        uploadMedia(apiKey, thumbnailBytes, "image/png") { thumbUploadResult ->
                            ...
                            createPost(
                                apiKey, caption, videoUrl, thumbnailUrl, thumbnailTimestampMs, scheduledAt, accounts.map { it.id }
                            ) { postResult ->
                                postResult.onSuccess {
                                    callback(Result.success(PublishResult(accounts.map { it.platform })))
                                }.onFailure { callback(Result.failure(it)) }
